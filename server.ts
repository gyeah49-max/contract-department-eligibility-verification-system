...
// Lazy initialize Gemini AI client to prevent startup crash if API key is missing
let aiClient: GoogleGenAI | null = null;
function getGeminiClient(): GoogleGenAI {
  if (!aiClient) {
    const apiKey = process.env.GEMINI_API_KEY;
    if (!apiKey) {
      console.warn("⚠️ GEMINI_API_KEY environment variable is not defined. Using simulated AI fallback.");
    }
...
// In-memory Database for demo environment
const students: Student[] = [
  {
    ...
    documents: {
      ...
      sme_cert: false, // 대기업은 중소기업확인서 불필요
...
// --- High Fidelity Mock Documents Content ---
// Used when testing scenarios, allowing real Gemini parsing!
...
async function startServer() {
  ...
  app.post("/api/rules/update", (req, res) => {
    ...
    res.json({ message: "검증 요건 규칙이 성공적으로 업데이트되었습니다." });
  });
  ...
  // 7. Core Verification Endpoint (uses Gemini SDK)
  app.post("/api/verify", async (req, res) => {
    ...
    // Determine document text content
    let documentText = "";
    let isMock = false;
    ...
    if (mockKey && MOCK_DOCUMENTS_DB[mockKey]) {
      ...
      // Real uploaded file (e.g., base64 encoded text or base64 PDF)
      // Since we don't install heavy native pdf-parse binaries (which can fail during builds),
      // we utilize Gemini's native multimodal capabilities!
      // If we send base64 data to Gemini, it parses it.
    ...
    try {
      ...
      // If no valid API key is present, we trigger a high-quality local fallback to keep things working robustly
      if (process.env.GEMINI_API_KEY === "MY_GEMINI_API_KEY" || !process.env.GEMINI_API_KEY) {
        // Mock fallback simulation logic based on documentText content
        ...
        // Construct high fidelity structural response
      ...
      // --- REAL GEMINI AUDIT LOGIC (Runs when real API key is supplied) ---
      let contentsInput: any = [];

      if (fileBase64) {
        // Extract real uploaded document content (supports images and PDFs)
...
Provide a strict, professional Korean audit result according to the requested JSON response schema.`,
        ...
        // Send Korean mock text database directly to Gemini for real text analysis
        const textPart = {
          text: `You are an auditing bot verifying eligibility for Korean Contract Departments (계약학과). 
...
Provide a strict, professional Korean audit result according to the requested JSON response schema.`,
  ...
  // 8. Override status manually (Admin tool)
  ...
  // --- Serve Frontend Application via Vite / Express.static ---
